...
def initial(x):
    return np.exp(3j*x*2*np.pi/2.02)
...
time = ax.text(.0, 1.2, '', fontsize=15)
line1, = ax.plot([], [], linewidth=2, color='b', label=r'$\mathrm{Re}(\Psi)$')
line2, = ax.plot([], [], linewidth=2, color='r', label=r'$\mathrm{Im}(\Psi)$')
