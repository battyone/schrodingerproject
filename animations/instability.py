...
def initial(x):
    ...
    return np.sqrt(2) * np.sin(x*5*np.pi)
...
t_rag = (0,0.04)
save = True
foo = qm_rk4.Wavefunction(initial, potential=V, dx=0.02, dt=t_int, trange=t_rag, order=2, boundaries=(0,0))
print("done!")
foo.solve()
print("done2!")
...
if save:
    ...
    ani.save('instability.mp4', writer=FFwriter)
